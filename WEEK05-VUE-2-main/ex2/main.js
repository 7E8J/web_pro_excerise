var app = new Vue({
    ...
    methods: {
      addToCart(product, i){
        if(!product.is_favorite) return;

        if(this.cart.find(item => item.title === product.title)){
          this.cart = this.cart.map(item => {
            if(item.title === product.title) item.amount++
    ...
    computed: {
      filterByCost(current, previous){
        if(!this.costFilter) return this.products;
        return this.products.filter(product => this.costFilter >= product.price);
      }
    },
    watch: {
